...
return new class extends Migration {
    public function up(): void
    {
        Schema::create('sales_orders', function (Blueprint $t) {
            ...
            $t->string('number')->unique()->nullable();       // SO number (optional auto-generator)
            $t->string('status')->default('sale');            // sale, delivered, done, cancel
        ...
        Schema::create('sales_order_items', function (Blueprint $t) {
            $t->id();
            $t->foreignId('order_id')->constrained('sales_orders')->cascadeOnDelete();
            $t->foreignId('product_id')->constrained('products')->restrictOnDelete();
            $t->decimal('qty', 18, 3);
            $t->string('uom', 32)->nullable();
            $t->decimal('unit_price', 18, 2);
            $t->decimal('discount', 18, 2)->default(0);   // nominal
            $t->decimal('tax_rate', 5, 2)->default(0);    // %
            $t->decimal('line_total', 18, 2)->default(0); // base - disc + tax
            $t->timestamps();
        });
    }
